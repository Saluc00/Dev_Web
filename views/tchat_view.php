...
<body>
    ...
    if ($_GET['id']) {
        ...
        $req = $db->query('SELECT * FROM salons WHERE salon_id = '. $idPage);
        $salonActuel = $req->fetch(); 

        include_once 'includes/salons.php';
        $req = $db->query('SELECT username, content FROM message WHERE fk_salon_id = '.$idPage);
        $messages = $req->fetchAll();
        ?>

        <div id="message">
            <h2>Canal <?= ucfirst($salonActuel['salon_nom']); ?></h2>

            <?php
        foreach ($messages as $message) {
            if ($message['username'] == $_SESSION['pseudo']) { ?>
            <p style="color: #e63946;"><strong>
                ...
                <div id="formMessage">
                    <form method="post" action=''>
                        ...
                        <button type="submit" class="btn btn-primary" value="Envoyer"
                            name="messagebutton">Envoyer</button>
                    </form>
                </div>
                <?php
        } else { ?>
